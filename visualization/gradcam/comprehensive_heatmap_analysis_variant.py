...
class Config:
    ...
    BATCH_SIZE = 1
    NUM_WORKERS = 0
    NUM_SAMPLES = 10  # 分析的样本数量 (如果指定了SAMPLE_INDICES或SAMPLE_FILENAMES，此参数将被忽略)
    SAMPLE_INDICES: Optional[List[int]] = None # 指定要分析的样本索引列表，例如 [0, 5, 10]。如果为None，则分析前NUM_SAMPLES个样本。
    SAMPLE_FILENAMES: Optional[List[str]] = None # 指定要分析的样本文件名列表，例如 ["image1.jpg", "image2.png"]。如果为None，则分析前NUM_SAMPLES个样本或SAMPLE_INDICES指定的样本。
...
def main():
    ...
    samples_to_analyze = []
    if config.SAMPLE_FILENAMES is not None:
        # 如果指定了样本文件名，则根据文件名查找索引
        print(f"根据文件名查找样本: {config.SAMPLE_FILENAMES}")
        filename_to_idx = {os.path.basename(item["image_path"]): i for i, item in enumerate(dataset.meta)}
        for filename in config.SAMPLE_FILENAMES:
            idx = filename_to_idx.get(filename)
            if idx is not None:
                samples_to_analyze.append((idx, dataset[idx]))
                print(f"找到样本: {filename} -> 索引 {idx}")
            else:
                print(f"警告: 样本文件名 {filename} 未在数据集中找到，已跳过。")
    elif config.SAMPLE_INDICES is not None:
        # 如果指定了样本索引，则只分析这些样本
        print(f"根据索引查找样本: {config.SAMPLE_INDICES}")
        for idx in config.SAMPLE_INDICES:
            try:
                samples_to_analyze.append((idx, dataset[idx]))
                print(f"找到样本索引: {idx}")
            except IndexError:
                print(f"警告: 样本索引 {idx} 超出数据集范围，已跳过。")
    else:
        # 否则，分析前NUM_SAMPLES个样本
        print(f"分析前 {config.NUM_SAMPLES} 个样本")
        for i in range(min(config.NUM_SAMPLES, len(dataset))):
            samples_to_analyze.append((i, dataset[i]))

    processed_sample_count = 0
    for original_idx, sample_data_raw in tqdm(samples_to_analyze, desc="分析样本"):
        sample_data = {
            'image': sample_data_raw['image'],
            'caption': sample_data_raw['caption'],
            'input_ids': sample_data_raw['input_ids'],
            'attention_mask': sample_data_raw['attention_mask'],
            'image_path': sample_data_raw['image_path']
        }
        
        try:
            # 分析样本
            sample_result = analyzer.analyze_sample(
                model_before_tent, model_after_tent, sample_data, original_idx
            )
            ...
            print(f"完成样本 {original_idx}: {os.path.basename(sample_data['image_path'])}")
            
        except Exception as e:
            print(f"分析样本 {original_idx} 时出错: {e}")
            continue
    ...
    print(f">>> 成功分析了 {processed_sample_count} 个样本")
